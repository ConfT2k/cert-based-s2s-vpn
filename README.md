...
Cisco CSR에 SSH로 접속한 후 인증서를 설치합니다.

> ⚠️ **중요**: ACM에서 내보낸 인증서와 개인키를 함께 사용하려면 **PKCS12 방식을 권장**합니다. Terminal enrollment 방식은 CSR에서 자체 생성한 키페어를 사용하므로 ACM 개인키와 호환되지 않습니다.

### 2.1 PKCS12를 사용한 인증서 임포트 (권장 방법)

ACM에서 내보낸 인증서와 개인키를 PKCS12 형식으로 변환하여 임포트하는 방식입니다.

#### 2.1.1 로컬 PC에서 PKCS12 파일 생성
...
```
프롬프트가 나타나면 Root CA 인증서 PEM 내용을 붙여넣고 `yes` 입력.
...
```
Subordinate CA 인증서 PEM 내용을 붙여넣고 `yes` 입력.
...
> 📌 PKCS12 임포트는 인증서, 개인키, CA 체인을 한 번에 가져오므로 가장 간편하고 오류가 적습니다.

---

### 2.2 Terminal Enrollment 방식 (대안 방법)

TFTP/SCP 환경이 없거나 수동 설정을 선호하는 경우 사용합니다.

> ⚠️ **주의**: 이 방식은 CSR에서 새로운 RSA 키페어를 생성하므로, ACM에서 내보낸 개인키는 사용되지 않습니다. 대신 CSR에서 CSR(Certificate Signing Request)을 생성하여 새 인증서를 발급받아야 합니다.
...
crypto pki authenticate AWS-ROOT-CA
```
Root CA 인증서 PEM을 붙여넣고 `yes` 입력.

#### 2.2.3 Subordinate CA Trustpoint 설정
...
crypto pki authenticate AWS-SUB-CA
```
Subordinate CA 인증서 PEM을 붙여넣고 `yes` 입력.

#### 2.2.4 디바이스 인증서 Trustpoint 설정
...
```
디바이스 인증서 PEM을 붙여넣습니다.

> ⚠️ 이 방식은 `rsakeypair AWS-VPN-KEY`로 지정한 키와 인증서의 공개키가 일치해야 합니다. ACM에서 발급받은 인증서를 그대로 사용하려면 PKCS12 방식을 사용하세요.
